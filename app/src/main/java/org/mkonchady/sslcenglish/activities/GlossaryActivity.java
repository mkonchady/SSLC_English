...
import org.mkonchady.sslcenglish.qa.GlossaryQuiz;
...
public class GlossaryActivity extends  AppCompatActivity {
    ...
    private GlossaryQuiz glossaryQuiz;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        glossaryQuiz = new GlossaryQuiz(context, Constants.NUM_QUESTIONS, lid);
        questions = glossaryQuiz.getQuestions();
        ...
        if (!reopened) {    // create a new quiz
            ...
            glossaryQuiz = new GlossaryQuiz(Constants.NUM_QUESTIONS, questions);
            glossaryQuiz.setCurrent_q(current_q);
        }
    ...
    private void showQuestion() {
        current_q = glossaryQuiz.getCurrent_q();
        final Question question = glossaryQuiz.getQuestion(current_q);
        String glossary = question.getQuery() + Constants.MEANS;
        meaningTV.setText(UtilsMisc.fromHtml(glossary));

        if (show_answer) {      // highlight the answer word in the glossary
            show_answer = false;
        }
    ...
    private void setButtons() {
        radio_g.setOnCheckedChangeListener(new RadioGroup.OnCheckedChangeListener() {
            public void onCheckedChanged(RadioGroup group, int checkedId) {
                if (button_set() ) {
                    ...
                    String message = statDB.updateStatTable(context, Constants.GLOSSARY_ACTIVITY, question, given_answer);
        ...
        View.OnClickListener pbLis = new View.OnClickListener() {
            ...
            public void onClick(View v) {
                current_q = glossaryQuiz.getCurrent_q();
                if (current_q > 0)
                    glossaryQuiz.setCurrent_q(current_q - 1);
                reset_buttons();
                answerText = Constants.UNDERSCORE;
                showQuestion();
        ...
        View.OnClickListener nbLis = new View.OnClickListener() {
            ...
            public void onClick(View v) {
                current_q = glossaryQuiz.getCurrent_q();
                if (current_q < glossaryQuiz.num_questions - 1)
                    glossaryQuiz.setCurrent_q(current_q + 1);
                reset_buttons();
                answerText = Constants.UNDERSCORE;
                showQuestion();
